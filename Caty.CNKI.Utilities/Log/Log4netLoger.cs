...
namespace Caty.CNKI.Utilities.Log
{ 
    public class Log4netLoger : ILoger
    {
        private static ILog loger = null;
        private static ILoggerRepository repository = LogManager.CreateRepository("CNKIRepository");
        public Log4netLoger()
        {
            try
            {
                if (loger == null)
                {
                    try
                    {
                        ...
                        loger = LogManager.GetLogger(repository.Name,"Spider");
                    }
                    catch
                    {
                        loger = LogManager.GetLogger(repository.Name,"Spider");
